...
import { getEscrowContract, getContractAddresses } from '@/lib/contracts'
...
export default function NewTaskPage() {
  ...
  const handleSubmit = async () => {
    ...
    const taskId = (BigInt(nextId as bigint) - 1n).toString()

    // Fund if requested
    if (formData.fundNow) {
      // Approve first
      const mockERC20 = await import('@/lib/contracts').then(m => m.getMockERC20Contract(walletClient))
      const escrowAddr = escrow.address
      await (await mockERC20).write.approve([escrowAddr, amount])
